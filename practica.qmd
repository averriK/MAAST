...
<!-- comentarios sobre esta señal en pago 931 de paper de Hilbert -->
...
ggplot() + geom_line(aes(x=t,y=s),color="blue",linewidth=1) +  labs(x = "t ",y = "s(t)")
```

### $s(t)=e^{-0.01t} cos(\frac{2\pi}{32}t ) \ \ \  0\leq t \leq 512$
<!-- comentarios de este ejemplo en Hilbert pag  943 -->
```{r message=FALSE,include=FALSE}
dt <- 3 #32/10
t <- seq(0,512,by=dt)
...
par(oma = c(0, 0, 0, 1), mar = c(5, 4, 4, 5) + 0.1)
plot(WT3, plot.cb = TRUE, plot.phase = FALSE,main="dog")
ggplot() + geom_line(aes(x=t,y=s),color="blue",linewidth=1) +  labs(x = "t ",y = "s(t)")+theme_bw()
```
...
ggplot() + geom_line(aes(x=t,y=X),color="blue",linewidth=1) + geom_line(aes(x=t,y=Y),color="red",size=0.7) +theme_bw()
```
